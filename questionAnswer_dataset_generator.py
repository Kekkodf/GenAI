...
def main():
    ...
    for name in dataset_name:
        data = read_file('./data/questionAnswer_dataset/'+name+'.txt')
        ...
        df.to_csv(f'./data/questionAnswer_dataset/{name}.csv', index=False)

    #concatenate all dataframes
    df = pd.concat([pd.read_csv('./data/questionAnswer_dataset/'+name+'.csv') for name in dataset_name])
    #drop duplicates
    df = df.drop_duplicates('Question')
    #add column with id
    df.insert(0, 'id', range(1, 1 + len(df)))
    #substitute yes for True and no for False as boolean values
    df['Answer'] = df['Answer'].replace({'yes': True, 'no': False, 'Yes': True, 'No': False})
    #save the final dataframe
    df.to_csv('./data/questionAnswer_dataset/dataset_Q&A.csv', index=False)
    for name in dataset_name:
        os.remove('./data/questionAnswer_dataset/'+name+'.csv')
